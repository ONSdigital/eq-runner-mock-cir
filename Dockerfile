...
RUN apt-get update \
    && apt-get install build-essential jq curl unzip -y --no-install-recommends gcc
...
CMD ["gunicorn", "app.main:app", "-b", "0.0.0.0:5004", "--worker-class", "uvicorn.workers.UvicornWorker", "--timeout", "0"]
